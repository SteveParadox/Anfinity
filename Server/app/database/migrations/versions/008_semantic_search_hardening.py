"""Align hybrid_search scoring with semantic-search feature requirements.

Revision ID: 008_semantic_search_hardening
Revises: 007_graph_clusters
...
revision = "008_semantic_search_hardening"
down_revision = "007_graph_clusters"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    # Restore the previous implementation from revision 003.
    op.execute(
        f"""
        CREATE OR REPLACE FUNCTION hybrid_search(
            p_query_text VARCHAR,
        ...
        scored_results AS (
            ...
            FROM merged_results
            WHERE similarity > 0.0 OR text_score > 0.0
        )
        SELECT
            note_id,
            title,
            ...
            SUBSTRING(content, POSITION(LOWER(p_query_text) IN LOWER(content)) - 30, 150) AS highlight
        FROM scored_results
        ORDER BY final_score DESC, created_at DESC
        LIMIT p_limit;
        $$ LANGUAGE SQL STABLE;
